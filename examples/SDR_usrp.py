...
# Generated: Mon Oct 17 00:07:11 2016
##################################################
...
class SDR_usrp(gr.top_block, Qt.QWidget):

    def __init__(self, miusrp):
        ...
        self.settings = Qt.QSettings("GNU Radio", "SDR_usrp")
        ...
        self.ledYellow = ledYellow = False
        self.ledRed = ledRed = False
        self.labelSensor = labelSensor = ''
        self.gain = gain = 30
        ...
        self.Tab.addTab(self.Tab_widget_2, 'Motas')
        ...
        self.sensor = qtgui.edit_box_msg(qtgui.STRING, '', '', True, True, 'sensor')
        self._sensor_win = sip.wrapinstance(self.sensor.pyqwidget(), Qt.QWidget)
        self.Tab_grid_layout_2 .addWidget(self._sensor_win,  1,1,1,2)
        ...
        _ledYellow_push_button = Qt.QPushButton('Led amarillo')
        self._ledYellow_choices = {'Pressed': True, 'Released': False}
        _ledYellow_push_button.pressed.connect(lambda: self.tfg_Aplication_0.toggleYellowLed())
        _ledYellow_push_button.released.connect(lambda: self.set_ledYellow(self._ledYellow_choices['Released']))
        self.Tab_grid_layout_2 .addWidget(_ledYellow_push_button,  0,2,1,1)
        _ledRed_push_button = Qt.QPushButton('Led rojo')
        self._ledRed_choices = {'Pressed': True, 'Released': False}
        _ledRed_push_button.pressed.connect(lambda: self.tfg_Aplication_0.toggleRedLed())
        _ledRed_push_button.released.connect(lambda: self.set_ledRed(self._ledRed_choices['Released']))
        self.Tab_grid_layout_2 .addWidget(_ledRed_push_button,  0,0,1,1)
        self._labelSensor_tool_bar = Qt.QToolBar(self)
        
        if None:
          self._labelSensor_formatter = None
        else:
          self._labelSensor_formatter = lambda x: x
        
        self._labelSensor_tool_bar.addWidget(Qt.QLabel('Sensor'+": "))
        self._labelSensor_label = Qt.QLabel(str(self._labelSensor_formatter(self.labelSensor)))
        self._labelSensor_tool_bar.addWidget(self._labelSensor_label)
        self.Tab_grid_layout_2 .addWidget(self._labelSensor_tool_bar,  1,0,1,1)
        ...
        self._MAC_etq_modoSrc_tool_bar.addWidget(Qt.QLabel("Modo direcc (origen)"+": "))
        ...
        self._MAC_etq_modoDest_tool_bar.addWidget(Qt.QLabel("Modo direcc (destino)"+": "))
        ...
        _LedGreen_push_button = Qt.QPushButton('Led verde')
        self._LedGreen_choices = {'Pressed': True, 'Released': False}
        _LedGreen_push_button.pressed.connect(lambda: self.tfg_Aplication_0.toggleGreenLed())
        _LedGreen_push_button.released.connect(lambda: self.set_LedGreen(self._LedGreen_choices['Released']))
        self.Tab_grid_layout_2 .addWidget(_LedGreen_push_button,  0,1,1,1)
        ...
        self.msg_connect((self.tfg_Aplication_0, 'out'), (self.sensor, 'val'))    
    ...
    def get_ledYellow(self):
        return self.ledYellow

    def set_ledYellow(self, ledYellow):
        self.ledYellow = ledYellow

    def get_ledRed(self):
        return self.ledRed

    def set_ledRed(self, ledRed):
        self.ledRed = ledRed

    def get_labelSensor(self):
        return self.labelSensor

    def set_labelSensor(self, labelSensor):
        self.labelSensor = labelSensor
        Qt.QMetaObject.invokeMethod(self._labelSensor_label, "setText", Qt.Q_ARG("QString", str(self.labelSensor)))
    ...
    def get_LedGreen(self):
        return self.LedGreen

    def set_LedGreen(self, LedGreen):
...
def main(usrp, top_block_cls=SDR_usrp, options=None):
